...
int main( int argc, char *argv[] ) {
  ...
  double *A_, *y, *y_orig;
  int blocks_per_process = (n+blk*size-1)/(blk*size); //calcula numero de bloques por proceso

 
  double *x = (double *) malloc( n*sizeof(double) );
  if( rank==0 ) {
    int m = blocks_per_process*blk*size;

    A_ = (double *) calloc( m*n, sizeof(double) );

    for( int i=0; i<n; i++ ) {
      A(i,i) = ( (double) rand() ) / RAND_MAX; //generar matriz aleatoria
      for( int j=i+1; j<n; j++ ) {
        A(i,j) = A(j,i) = ( (double) rand() ) / RAND_MAX; //igual wur snyrd
      }
    }
    for( int i=0; i<n; i++ ) {
  ...
  //si error 0 entonces ejecucion adecuada
  if( rank == 0 ) {
    for( int i=0; i<n; i++ ) {
      y[i] -= y_orig[i];
    }
