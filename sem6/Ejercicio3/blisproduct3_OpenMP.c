...
int main(int argc, char *argv[]){
  ...
  /*************************************************************************/
  double elapsed_par, ucpu_par, scpu_par;
  ctimer(&elapsed_par, &ucpu_par, &scpu_par);
  #pragma omp parallel for collapse(2) private(Bc, Ac)
            		...
            		for (int j = jc; j < jc + nc; j++) {
                		for (int p = pc; p < kc + pc; p++) {
                    			for (int i = ic; i < ic + mc; i++) {
                    				#pragma omp atomic	
                           			C(i, j) += Ac(i - ic, p-pc) * Bc(p-pc, j-jc);
                    			}
                		}
            		}
        	}
