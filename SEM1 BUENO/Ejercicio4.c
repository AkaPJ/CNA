...
int main( int argc, char *argv[] ) {
  ...
  double start_tori, end_tori;
  //Version original
  start_tori=MPI_Wtime();
  int k_original = potencia_original( n, A, &lambda, q );
  end_tori=MPI_Wtime();
  double tori = end_tori - start_tori;
  ...
  //Tiempo versión colectiva
  double start_tcol, end_tcol;
  ...
  start_tcol=MPI_Wtime();
  int k_colectiva = potencia_colectiva( n, A_colectiva, &lambda_colectiva, q_colectiva );
  end_tcol=MPI_Wtime();	
  double tcol= end_tcol-start_tcol;
  if( rank==0 ) {
